...
func (s *Server) initI18n(engine *gin.Engine) error {
	...
	findI18nParamNames := func(key string) []string {
		...
		for i := 0; i < keyLen-1; i++ {
			if key[i:i+2] == "{{" { // Detect the beginning "{{"
				j := i + 2
				isFind := false
				for ; j < keyLen-1; j++ {
					if key[j:j+2] == "}}" { // End "}}"
...
func (s *Server) startTask() {
	...
	go func() {
		...
		// The traffic is counted every 10 seconds, and the first startup is delayed by 5 seconds, which is staggered with the time of restarting xray
		s.cron.AddJob("@every 10s", job.NewXrayTrafficJob())
	}()

	// Check for inbound traffic exceeding and expiration every 30 seconds
